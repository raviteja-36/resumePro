...
if (!token || !geminiApiKey) {
    ...
    console.error('Please ensure TELEGRAM_BOT_TOKEN and GEMINI_API_KEY are set in your .env file');
    process.exit(1);
}
...
bot.on('callback_query', async (query) => {
    ...
    switch (action) {
        case 'upload_resume':
            bot.sendMessage(chatId, '📤 Please upload your resume in PDF format');
            break;
            
        case 'ats_score':
            bot.sendMessage(chatId, '📊 To check your ATS score, please upload your resume (PDF)');
            break;
        ...
        case 'generate_questions':
            ...
            bot.sendMessage(chatId, '❓ What type of questions would you like?', options);
            break;
        ...
        case 'analyze_resume':
            bot.sendMessage(chatId, '🔍 For detailed resume analysis, please upload your resume (PDF)');
            break;
        ...
        case 'generate_from_resume':
            bot.sendMessage(chatId, '📄 To generate questions from your resume, please upload it (PDF)');
            break;
...
async function startMockInterview(chatId) {
    ...
    if (questionsArray.length > 0) {
        ...
        bot.sendMessage(chatId, `❓ Question 1/8:\n\n${questionsArray[0]}`);
    } else {
        bot.sendMessage(chatId, '⚠️ Could not generate questions. Please try again.');
        userStates.delete(chatId);
    }
}

async function generateGeneralQuestions(chatId) {
    bot.sendMessage(chatId, '⏳ Generating general interview questions...');
    ...
    sendFormattedResponse(chatId, '📝 *General Interview Questions*\n\n' + questions);
}

async function generateTechnicalQuestions(chatId) {
    bot.sendMessage(chatId, '⏳ Generating technical interview questions...');
    
    const prompt = `Generate 10 challenging technical interview questions covering:\n
    - Data structures & algorithms\n- System design\n- Language-specific concepts\n- Debugging scenarios\n\nFormat as a numbered list.`;
    
    const questions = await generateAIContent(prompt);
    sendFormattedResponse(chatId, '💻 *Technical Interview Questions*\n\n' + questions);
}

async function generateBehavioralQuestions(chatId) {
    bot.sendMessage(chatId, '⏳ Generating behavioral interview questions...');
    
    const prompt = `Generate 10 behavioral interview questions focusing on:\n
    - Team conflicts\n- Leadership\n- Failure experiences\n- Time management\n- Work ethics\n\nFormat as a numbered list.`;
    
    const questions = await generateAIContent(prompt);
    sendFormattedResponse(chatId, '🤝 *Behavioral Interview Questions*\n\n' + questions);
}
...
bot.on('document', async (msg) => {
    ...
    if (!msg.document.mime_type.includes('pdf')) {
        return bot.sendMessage(chatId, '⚠️ Please upload a PDF file only.');
    }
    ...
    const processingMsg = await bot.sendMessage(chatId, '⏳ Processing your resume...');
    ...
    try {
        ...
        await bot.deleteMessage(chatId, processingMsg.message_id);
        bot.sendMessage(chatId, '⚠️ Error processing your resume. Please ensure it\'s a valid PDF and try again.');
    } finally {
...
async function handleResumeActions(chatId, action, query) {
    ...
    switch (action) {
        case 'action_ats':
            ...
            sendFormattedResponse(chatId, atsScore);
            break;
            
        case 'action_generate_questions':
            if (extractedSkills.length === 0) {
                return bot.sendMessage(chatId, '⚠️ No skills detected to generate specific questions. Try general questions instead.');
            }
            
            bot.sendMessage(chatId, '⏳ Generating tailored interview questions...');
            ...
            const questions = await generateAIContent(prompt);
            sendFormattedResponse(chatId, '🎯 *Tailored Interview Questions*\n\n' + questions);
            break;
            
        case 'action_content_analysis':
            bot.sendMessage(chatId, '🔍 Analyzing your resume content...');
            ...
            sendFormattedResponse(chatId, '📝 *Resume Analysis Report*\n\n' + analysis);
            break;
            
        case 'action_optimize_role':
            userStates.set(chatId, { ...userState, state: 'WAITING_FOR_JOB_TITLE' });
            bot.sendMessage(chatId, '💼 Please reply with the *exact job title* you\'re targeting (e.g., "Senior Frontend Developer"):', 
                { parse_mode: 'Markdown' });
            break;
    }
}
...
async function handleJobTitleInput(chatId, jobTitle, userState) {
    bot.sendMessage(chatId, `⏳ Optimizing your resume for "${jobTitle}"...`);
    ...
    sendFormattedResponse(chatId, `💼 *Optimization for ${jobTitle}*\n\n${optimization}`);
    userStates.delete(chatId);
}
...
async function handleMockInterviewResponse(chatId, answer, userState) {
    ...
    if (nextIndex < questions.length) {
        ...
        bot.sendMessage(chatId, `❓ Question ${nextIndex + 1}/${questions.length}:\n\n${questions[nextIndex]}`);
    } else {
        endMockInterview(chatId);
    }
}
...
function endMockInterview(chatId) {
    ...
    if (userState?.state === 'MOCK_INTERVIEW') {
        bot.sendMessage(chatId, '🎉 *Mock Interview Completed!*\n\nReview your answers and feedback. Practice makes perfect!\n\nType /start to explore other features.', 
            { parse_mode: 'Markdown' });
        userStates.delete(chatId);
    } else {
        bot.sendMessage(chatId, '⚠️ No active mock interview to end.');
    }
}

function sendFormattedResponse(chatId, message) {
    const chunks = splitMessage(message);
    chunks.forEach((chunk, index) => {
        // Add header to first chunk only
        const formatted = index === 0 ? chunk : chunk;
